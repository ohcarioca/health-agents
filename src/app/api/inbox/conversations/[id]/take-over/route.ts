...
export async function POST(
  ...
  if (updateError) {
    console.error("[inbox/take-over] update error:", updateError.message);
  ...
  const { error: msgError } = await admin.from("messages").insert({
    ...
    metadata: { sent_by: user.id, action: "take_over" },
  });

  if (msgError) {
    console.error("[inbox/take-over] system message error:", msgError.message);
  }
